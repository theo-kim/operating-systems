...
void insert(int key, int val) {
    ...
    // ONLY LOCK THE CURRENT BUCKET FROM BEING ALTERED
    pthread_mutex_lock(&bucket_locks[i]);
    e->next = table[i];
    e->key = key;
    e->val = val;
    table[i] = e;
    // UNLOCK CURRENT BUCKET
...
bucket_entry * retrieve(int key) {
    bucket_entry *b;
    int i = key % NUM_BUCKETS;
    // NO NEED FOR THIS LOCK AS A READ / READ SIMULTANEOUSLY HAPPENNING DOES
...
int main(int argc, char **argv) {
    ...
    // Initialize Locks for Writing to each bucket
    for (int iter = 0; iter < NUM_BUCKETS; ++iter) {
        pthread_mutex_init(&bucket_locks[iter], NULL);
    }
